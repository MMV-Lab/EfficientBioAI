...
cfg_path = 'configs/omnipose/omnipose.yaml'
exp_path = 'experiment/instanceseg_opv_int8_prune'
...
{
    'exclude': True,
    'op_names': ['output.2','output','output.1','downsample.down.res_down_3.conv.conv_3.2']
}]
customized_replace_func = {
    'MergeBlock': lambda module, mask: module,
    'UpsamplingBilinear2d': lambda module, mask: module
}
device = torch.device("cpu" if quantization_config.quantization.backend == 'openvino' else "cuda")
dummy_input = torch.rand(1,*data_cfg.input_size).to(device)
pruner = L1NormPruner(model.net, config_list,mode='dependency_aware',dummy_input=dummy_input)
...
ModelSpeedup(model.net, torch.rand(1,*data_cfg.input_size).to(device), masks,customized_replace_func=customized_replace_func).speedup_model()
#=====fine tuning=====:
# only for cellpose:
test_dir = None 
train_dir = '/home/ISAS.DE/yu.zhou/EfficientBioAI/data/omnipose/in_house_data/train'
img_filter = None
mask_filter = '_masks'
model_dir = exp_path
output = io.load_train_test_data(train_dir, test_dir,img_filter, mask_filter, False, False)
...
cpmodel_path = model.train(images, labels, train_files=image_names,
                                           ...
                                           channels=[1,0],
                                           save_path=os.path.realpath(model_dir), 
                                           save_every=1000,
                                           save_each=False,
                                           n_epochs=1000,
                                           batch_size=8, 
