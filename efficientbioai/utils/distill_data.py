...
def getDistilData(teacher_model,size, device):
    ...
    layers = sum([
        1 if isinstance(layer, nn.BatchNorm2d) or isinstance(layer, nn.BatchNorm3d) else 0
    ...
    for n, m in teacher_model.named_modules():
        if (isinstance(m, nn.Conv2d) or isinstance(m,nn.ConvTranspose2d) or \
            isinstance(m, nn.Conv3d) or isinstance(m,nn.ConvTranspose3d)) and len(hook_handles) < layers:
    ...
    input_mean = torch.zeros(1, 1).to(device)
    input_std = torch.ones(1, 1).to(device)
