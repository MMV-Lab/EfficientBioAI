...
''''/ddn/support/soft/miniconda3/envs/efficientbioai/lib/python3.8/site-packages/
    cellpose/core.py 第315行的X.requires_grad = True是新增的'''
...
def getDistilData(teacher_model,size):
    ...
    for it in range(1):
        ...
        tmp_mean = torch.mean(gaussian_data.view(gaussian_data.size(0), 2,
                                                     -1),
                                  dim=2)
        tmp_std = torch.sqrt(
                torch.var(gaussian_data.view(gaussian_data.size(0), 2, -1),
                          dim=2) + eps)
