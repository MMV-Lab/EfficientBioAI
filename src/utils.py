...
# adapted from https://stackoverflow.com/questions/6866600/how-to-parse-read-a-yaml-file-into-a-python-object
...
def pad(img: torch.Tensor, N) -> torch.Tensor:
    """for calibration. Unet requires size of input to be muptiples of 2^N, where N is the depth of the network.
    ...
    """
    z,y,x =  img.shape[-3:]
    f = lambda l,N:(2^N)*(l//2^N) if (l%(2^N))<=(2^(N-1)) else (2^N)*(l//2^N + 1) #to nearest multiples of 2^N
